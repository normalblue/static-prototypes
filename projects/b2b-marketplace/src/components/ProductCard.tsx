...
export function ProductCard({ product, compact = false }: ProductCardProps) {
  ...
  if (compact) {
    return (
      <div className="flex flex-col bg-white rounded-xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow p-3 min-w-[200px] cursor-pointer group">
        <div className="relative h-24 mb-3 overflow-hidden rounded-lg bg-slate-50">
          <img src={product.image} alt={product.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
        </div>
        <h4 className="text-sm font-medium text-slate-900 line-clamp-2 mb-1">{product.name}</h4>
        ...
        <div className="mt-auto flex items-end justify-between">
          ...
          <button 
            onClick={(e) => { e.stopPropagation(); addToCart(e, product.image); }}
            className="w-8 h-8 rounded-full bg-brand-50 text-brand-600 flex items-center justify-center hover:bg-brand-500 hover:text-white transition-colors"
          >
    ...
    <div className="flex flex-col bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden group cursor-pointer relative">
      ...
      <div className="p-4 flex flex-col flex-1">
        <div className="flex justify-between items-start mb-2">
          <p className="text-xs font-medium text-brand-600 bg-brand-50 px-2 py-0.5 rounded-md">{product.category}</p>
          <div className="flex items-center text-amber-400">
            <Star className="w-3 h-3 fill-current" />
            <span className="text-xs font-medium text-slate-600 ml-1">{product.rating}</span>
          </div>
        </div>
        <h3 className="text-base font-semibold text-slate-900 leading-tight mb-1 group-hover:text-brand-600 transition-colors line-clamp-2">{product.name}</h3>
